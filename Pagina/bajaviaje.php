<?php
	...
	$sqltoken = mysqli_query($coneccion, "SELECT * FROM viajes WHERE id='".$_GET['id']."'");
...
//califico negativamente al usuario si acepto pasajeros(En proceso)
$acepto = mysqli_query($coneccion, "SELECT * FROM postulaciones WHERE viajes_id='".$_GET['id']."', estado='".A."'");
if ($acepto) {

	$calificacion = $user['calificacion'] -5;
	$cantidad = $user['cantidad_votos'] +1;

	$sql = mysqli_query($coneccion, "UPDATE usuarios SET  calificacion='".$calificacion."', cantidad_votos='".$cantidad."' WHERE id=".$user['id']);
	if($sql){
	$exito = true;
	}else{
	$exito = false;
	...
	$alta  = mysqli_query($coneccion, "INSERT INTO viajes_finalizados (preciototal, origen, destino, fecha, duracion, horario, plazas,  vehiculos_id, usuarios_id) VALUES ('".$viaje['preciototal']."', '".$viaje['origen']."', '".$viaje['destino']."', '".$viaje['fecha']."','".$viaje['duracion']."','".$viaje['plazas']."','".$viaje['vehiculos_id']."','".$viaje['usuarios_id']."')");
	...
	if(!$alta) {//si la transaccion fallo
		...
		$baja = mysqli_query($coneccion, "DELETE FROM viajes WHERE id=".$_GET['id']); //delete el viaje de la tabla de viajes aun pendientes
